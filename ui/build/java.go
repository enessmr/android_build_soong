...
func checkJavaVersion(ctx Context, config Config) {
	...
	if runtime.GOOS == "linux" {
		...
		if strings.Contains(java_version, "openjdk") {
			...
			//ctx.Fatalln("stop")
			ctx.Println("*******************************************************")
			ctx.Println("idc what java u use lol, just say fuck you to oracle ")
			ctx.Println("jdk cuz closed source bad for linux")
			ctx.Println("anyways i will let u continiuu")
			ctx.Println("*******************************************************")
		}
	}
